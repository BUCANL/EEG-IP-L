...
function [com] = new_viewprops( EEG, typecomp, chanorcomp, spec_opt, erp_opt, scroll_event, classifier_name, fig)
...
% Subdivide the components into lists by PLOTPERFIG
allList = 1:length(chanorcomp);
possibleDivisions = ceil(length(chanorcomp) / PLOTPERFIG);
subList = cell(possibleDivisions,1);

for i = 1 : possibleDivisions;
    cappedSub = fastif(i+((i-1)*PLOTPERFIG)+PLOTPERFIG > length(chanorcomp),length(chanorcomp),i+((i-1)*PLOTPERFIG)+PLOTPERFIG);
    stepList = allList(i+((i-1)*PLOTPERFIG):cappedSub);
    subList{i} = stepList;
end
% subList now holds N-many lists of components to draw.

% Build array of figures/figure handles
allHandles = build_viewprop_handles(EEG, possibleDivisions, subList{1}, typecomp);
disp('hold');
for ri = 1 : length(subList{1}); % For each possible component...
    for subFig = 1 : length(subList); % in each figure...
        if(i <= length(subList{subFig})); % if there are still components to draw...
            single_plot_viewprop(EEG, typecomp, subList{subFig}, ...
                spec_opt, erp_opt, scroll_event, classifier_name, allHandles{subFig},ri);
        end
    end
end
