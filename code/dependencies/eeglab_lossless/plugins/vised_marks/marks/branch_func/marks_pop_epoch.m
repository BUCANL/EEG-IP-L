...
%	'startpnt' - ['yes'|'no'] add 'startpnt' field to epoch structure. 'startpnt'
%				 holds the data point index into the continuous data where eachepoch
%				 began {default: 'yes'}
%   
% Outputs:
...
try, g.startpnt;          catch, g.startpnt = 'yes'; end;
...
if strcmp(g.startpnt,'yes')
    %add "startpnt" field to "epoch" structure...
    for i=1:EEG.trials;
        EEG.epoch(i).startpnt=alllatencies(i)+reallim(1);
    end
end
