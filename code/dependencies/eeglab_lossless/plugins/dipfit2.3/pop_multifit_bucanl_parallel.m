% pop_multifit_bucanl_parallel.m() - fit multiple component dipoles using DIPFIT 
%
% Usage:
...
function [EEG, com] = pop_multifit_bucanl_parallel(EEG, comps, threads, varargin);
    ...
    % -----------------------------
    logging_log('INFO', 'Searching dipoles locations...');
    chansel =  EEG.dipfit.chansel;
    %elc     = getelecpos(EEG.chanlocs, EEG.dipfit);
    plotcomps = [];

    eegmodels = pararrayfun(threads, ...
        @(i) fit_function_single(EEG.dipfit.model(i), EEG, i, g, defaultconstraint), ...
        comps(:)', 'UniformOutput', false);
    EEG.dipfit.model = cell2mat(eegmodels(cellfun(@(x) ~isempty(x), eegmodels)));
...
function EEGMODEL = fit_function_single(EEGMODEL, EEG, i, g, defaultconstraint)
    if i <= length(EEG.dipfit.model) && ~isempty(EEGMODEL.posxyz)
        if g.dipoles == 2,
            % try to find a good origin for automatic dipole localization
            EEGMODEL.active = [1 2];
            EEGMODEL.select = [1 2];
            if isempty(EEGMODEL.posxyz)
                EEGMODEL.posxyz = zeros(1,3);
                EEGMODEL.momxyz = zeros(2,3);
            else
                EEGMODEL.posxyz(2,:) = EEG.dipfit.model(i).posxyz;
                if strcmpi(EEG.dipfit.coordformat, 'MNI')
                     EEGMODEL.posxyz(:,1) = [-40;40];
                else
                    EEGMODEL.posxyz(:,2) = [-40;40];
                end;
                EEGMODEL.momxyz(2,:) = EEGMODEL.momxyz;
            end;
        else
            EEGMODEL.active = [1];
            EEGMODEL.select = [1];
        ...
        try,
            % REMOVED BELOW LINE AS PARALLEL LIBRARY DOES NOT LIKE
            % THE FACT THAT IT COUNTS AS AN EXTERNAL MODIFICATION
            % EEG.dipfit.model(i) = EEGMODEL;
            if g.dipoles == 2,
                TEEG = dipfit_nonlinear(EEG, 'component', i, 'symmetry', 'x');
            else
                TEEG = dipfit_nonlinear(EEG, 'component', i, 'symmetry', []);
            end;
            EEGMODEL = TEEG.dipfit.model(i);
        catch, 
            EEGMODEL.rv = NaN; disp('Maximum number of iterations reached. Fitting failed');
        end;
