...
% TODO(brad)
% This seems to be a programmers error...
% it looks like the standard != null && <do something> idiom
% but they are using single &
% in octave this causes an error but I'm not sure why it works
% in matlab
if isfield(EEG.dipfit, 'elecfile') && ~isempty(EEG.dipfit.elecfile)
%if isfield(EEG.dipfit, 'elecfile') & ~isempty(EEG.dipfit.elecfile)
    cfg.elecfile = EEG.dipfit.elecfile; %% Error line!
end
