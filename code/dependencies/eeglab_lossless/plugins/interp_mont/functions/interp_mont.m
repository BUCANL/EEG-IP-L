...
%
% Usage:
%   >>  EEG = interp_mont(EEG, coordfname, varargin);
%
% Inputs:
...
% Structure switch so that the current structure is the old version
%swap.chanlocs = [];
%swap.chanlocs  = tmp.chanlocs;
%tmp.chanlocs = EEG.chanlocs;
%EEG.chanlocs = swap.chanlocs;
%swap.chanlocs = [];

EEG.nbchan=length(EEG.chanlocs);

% The submit structure to interp is the old + the new
EEG = eeg_interp(EEG,[1:nintrplocs],'spherical');

% The first set indecies of data and channels are the interpolated ones
% Keepo these cut out the rest
EEG.data = EEG.data(1:nintrplocs,:,:);
EEG.chanlocs = EEG.chanlocs(1:nintrplocs);
