% EEG = pop_warp_locs() - Collect inputs for intrpmont. Interpolate current
%      data to coordinates from a specified coordinate file.
%
% Usage:
%   >>  EEG = pop_interpmont(EEG, coordfname, varargin);
%
% Inputs:
...
function [EEG com] = pop_warp_locs(EEG, coordfname, varargin)
...
if isempty(optstr);
    com = sprintf('EEG = pop_warp_locs( %s, ''%s'');', inputname(1), coordfname)
else
    com = sprintf('EEG = pop_warp_locs( %s, ''%s'', %s);', inputname(1), coordfname, optstr)
end
...
if isempty(optstr);
    execcom = sprintf('EEG = eeg_warp_locs( %s, ''%s'');', inputname(1), coordfname);
else
    execcom = sprintf('EEG = eeg_warp_locs( %s, ''%s'', %s);', inputname(1), coordfname, optstr)
end
