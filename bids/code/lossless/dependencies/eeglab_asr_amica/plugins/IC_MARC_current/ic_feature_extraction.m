...
% In this section we had to convert some misuses of containers.Map to
% struct based lookups and a cellarray for the other containers.Map
% this is compatible with Octave and likely, more efficient

%run_functions = containers.Map(functions, repmat({0},1,length(functions)));
% Convert to struct
run_functions = struct();
for i=1:numel(functions)
    run_functions.(functions{i}) = false;
end

% arrayfun below just creates a cell array of 1:length(functions)
%feature_calculators = containers.Map(...
%    arrayfun(@(x){x}, 1:length(functions)),...
%    functions);
% If we have a map from ints -> strings why don't we just use a cell array
feature_calculators = arrayfun(@(x){char(functions(x))}, 1:length(functions));

if ( (strcmp(class(features), 'double') && all(round(features)==features)) || iscell(features))
    features_to_calculate=parse_feature_specification(features, uint32(numel(feature_calculators)));
else
...
if max(features_to_calculate) > numel(feature_calculators)
    error(['ic_feature_extraction: The following numbers do not have corresponding features defined: '...
        num2str(features_to_calculate(features_to_calculate > numel(feature_calculators)))])
end
...
%     [12, 6:7, 11, 15, 21, 23, 25, 27, 30:31], [1:2 14 16:20 26 29:31]});


feature_map = struct();
k = {'all', 'All','established_features',  'established_feature_set', 'established_spatial_features', 'spatial2'};
v = {1:nfeatures,1:nfeatures, establishedfeatureset, ...
    establishedfeatureset,...
    [1, 2, 6, 7, 11, 15, 21, 23, 25, 27, 30, 31], [1 2 14 16 17 18 19 20 26 29 30 31]};
for i = 1:numel(k)
    feature_map.(k{i}) = v{i};
end
