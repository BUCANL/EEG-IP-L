...
predprob = mnrval_octave(mod, featsstand);
...
% mnrval_octave is functionally equivalent to mnrval with two arguments
% of the type we are familiar with in ICMARC_run and the 

function pred = mnrval_octave(beta, x)
dataClass = 'double';
...
if pstar ~= 1 + p
    error('Rows of beta must be 1 more than columns of x');
end
k = size(beta,2) + 1;
if p > 0
