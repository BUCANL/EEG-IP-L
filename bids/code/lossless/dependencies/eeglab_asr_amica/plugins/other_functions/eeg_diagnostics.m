...
%ch_sd time
s='[sd_t_meth]';
sd_t_meth=batch_config(1).replace_string{find(strncmp(batch_config(1).replace_string,s,length(s)))}(length(s)+3:end-1);
s='[sd_t_vals]';
...
%ch_sd channel
s='[sd_ch_meth]';
sd_ch_meth=batch_config(1).replace_string{find(strncmp(batch_config(1).replace_string,s,length(s)))}(length(s)+3:end-1);
s='[sd_ch_vals]';
...
%low_r channel
s='[r_ch_meth]';
r_ch_meth=batch_config(1).replace_string{find(strncmp(batch_config(1).replace_string,s,length(s)))}(length(s)+3:end-1);
s='[r_ch_vals]';
...
%bridge channel
mr = median(EEG.data_r_ch,2);
iqrr = iqr(EEG.data_r_ch,2);
msr = mr./iqrr;
figure;
plot(msr);
hold on;
...
%low_r time
s='[r_t_meth]';
r_t_meth=batch_config(1).replace_string{find(strncmp(batch_config(1).replace_string,s,length(s)))}(length(s)+3:end-1);
s='[r_t_vals]';
...
%ic_sd1 time
s='[sd_t_meth]';
sd_t_meth=batch_config(3).replace_string{find(strncmp(batch_config(3).replace_string,s,length(s)))}(length(s)+3:end-1);
s='[sd_t_vals]';
...
%ic_sd2 time
s='[sd_t_meth]';
sd_t_meth=batch_config(7).replace_string{find(strncmp(batch_config(7).replace_string,s,length(s)))}(length(s)+3:end-1);
s='[sd_t_vals]';
